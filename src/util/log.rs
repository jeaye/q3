...
static tls_key: local_data::Key<Log> = &local_data::Key;
...
impl Log
{
  ...
  pub fn get_mut<T>(handler: &fn(&mut Log) -> T) -> T
  { 
    ...
    |opt|
    {
      match opt
      {
        ...
        None => fail!("Singleton not available: Log")
      }
    })
  }
  ...
  {
    let verbosity = do Log::get_mut |log| { log.verbosity };
